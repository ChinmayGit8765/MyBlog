<p align="center"><img src="docs/assets/hero.svg" alt="MyBlog — a personal blog, headed for Azure, this is the placeholder" width="100%"></p>

<p align="center">
  <b>My personal blog — it will get deployed onto Azure and host my cool reviews. This repo is the placeholder before that build starts.</b>
</p>

<p align="center">
  <img src="https://img.shields.io/badge/status-placeholder-lightgrey?style=flat-square" alt="status: placeholder">
  <img src="https://img.shields.io/github/stars/ChinmayGit8765/MyBlog?style=flat-square" alt="stars">
</p>

## ✨ What this is

- A personal blog for reviews and writing, intended to deploy on **Azure**.
- No app exists here yet — this repo currently holds this README and the assets under `docs/assets/`.
- Until the Azure build lands, active writing lives in the studio site linked below.

## 🎬 See it

<p align="center"><img src="docs/assets/map.svg" alt="Two places, one intent: writing lives on Exaryn Studio notes today; this repo is reserved for an Azure-hosted blog that is not built yet" width="100%"></p>
<p align="center"><sub>Honest layout: nothing in this tree except the README and these assets.</sub></p>

## 🧠 How it is supposed to work

```mermaid
flowchart LR
    NOW["Exaryn Studio notes<br/>writing that exists today"] -.-> THIS["This repo<br/>placeholder only"]
    THIS -->|"not built yet"| AZ["Azure-hosted blog<br/>reviews and writing"]
```

There is no application here. The diagram is the plan, not a running system.

## 🧭 Status

| | |
|---|---|
| **Stage** | 🚧 Placeholder — no code, no deployment yet |
| **What exists** | This README and the diagram above |
| **What's next** | An Azure-hosted blog for reviews and writing |

No fake roadmap beyond that: there is no build timeline yet, just the intent stated in the repo description.

## ✍️ Where the writing actually is today

While this repo is empty, current writing lives on the studio site:

**[Exaryn Studio notes](https://chinmaygit8765.github.io/exaryn-studio/)**
<br><sub>Where notes and writing currently live, ahead of this blog's own deployment.</sub>

## 🗂️ Project layout

```
MyBlog/
├── README.md          ← you are here
└── docs/assets/        hero banner + placeholder map
```

<p align="center"><sub>Built by <a href="https://github.com/ChinmayGit8765">Chinmay</a> · part of the <a href="https://chinmaygit8765.github.io/exaryn-studio/">Exaryn</a> studio</sub></p>
